...
class ServicesController extends Controller
{
    public static function getAllServices($status) {
        if ($status == "ALL") {
            $sql = "SELECT `productLineId`,`name`,`description`,`img`,`status` FROM product_line";
        } else {
            $sql = "SELECT `productLineId`,`name`,`description`,`img` FROM product_line WHERE `status` = 'ACTIVO'";
        }
    ...
    public static function updateServiceStatus(Request $request) {
        ...
        $update = DB::update('UPDATE product_line
                                    SET `status` = ? 
    ...
    public static function insertService(Request $request) {
        ...
        if(move_uploaded_file($_FILES["img"]["tmp_name"], $target_file)) {
            ...
            $sql = "INSERT INTO product_line(`name`,`description`,`img`,`date`,`status`) VALUES (?,?,?,NOW(),'ACTIVO')";
